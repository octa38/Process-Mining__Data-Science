...
Bottleneck analysis. Computes global and local average durations per event across all logs, builds the process-flow graph, finds the critical path, and selects the log with the shortest overall time. Produces the process flow diagram, the local-vs-global duration comparison chart, and the bottleneck identification.
Z3 initialisation. A sanity check that the solver is available.
Job-shop scheduling. Schedules four jobs across four workstations using the durations computed in cell 1. Depends on `local_average_durations` existing, so cell 1 must have run.
Gantt chart. Visualises the schedule from cell 3.
Live conformance checking. Interactive. See below.
---
